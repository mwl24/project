...
dog <- WCE(as.data.frame(chicken), analysis = "Cox", cutoff = 250, nknots = 1:3, 
    constrained = "R", aic = FALSE, MatchedSet = NULL,
    id = "id", 
    event = "concessions", 
    start = "Start", stop = "Stop", expos = "tactics",
    covariates = c("geo_scope"))
summary(dog)
dog
plot(dog)
nonusers <- rep(0, 250)
users <- rep(3, 250)
# for all models
HR.WCE(dog, users, nonusers, allres = TRUE)
chicken$geo_scope <- as.integer(chicken$geo_scope)

chicken <- df_pwp %>% group_by(id) %>% 
  mutate(Stop = row_number()) %>% 
  mutate(Start = Stop - 1)
df_pwp
as.data.frame(df_pwp[, c("id", "concessions", "Start", "Stop", "tactics", "geo_scope")])
drugdata %>% 
  group_by(Id) %>% 
  mutate(count = row_number()) %>% 
  slice_tail()
length(df_pwp$tactics)
coef.WCE(dog)
sum(is.na(df_pwp$tactics))
campaign_df[, c("id", "concessions", "Start", "Stop", "tactics")]
```
...
WCE(as.data.frame(campaign_df[1:100,]), analysis = "Cox", cutoff = 1000, nknots = 1:3, id = "id", 
    event = "concessions", 
    start = "Start", stop = "Stop", expos = "tactics")
